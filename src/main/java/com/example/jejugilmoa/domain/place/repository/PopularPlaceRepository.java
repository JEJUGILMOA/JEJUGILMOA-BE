...
public interface PopularPlaceRepository extends JpaRepository<PopularPlace, Long> {
    ...
    @Query(value = "SELECT pp FROM PopularPlace pp JOIN FETCH pp.place pl ORDER BY CASE WHEN pl.imageUrl IS NULL THEN 1 ELSE 0 END ASC, pp.visitCount DESC, pp.id ASC",
           countQuery = "SELECT COUNT(pp) FROM PopularPlace pp")
    Page<PopularPlace> findAllWithPlaceOrderByVisitCountDesc(Pageable pageable);

    @Query(value = "SELECT pp FROM PopularPlace pp JOIN FETCH pp.place pl WHERE pl.category.name = :categoryName ORDER BY CASE WHEN pl.imageUrl IS NULL THEN 1 ELSE 0 END ASC, pp.visitCount DESC, pp.id ASC",
    ...
    @Query("SELECT pp FROM PopularPlace pp JOIN FETCH pp.place pl JOIN FETCH pl.category WHERE pp.curationLabel = :label ORDER BY CASE WHEN pl.imageUrl IS NULL THEN 1 ELSE 0 END ASC, pp.visitCount DESC, pp.id ASC")
    List<PopularPlace> findByCurationLabelWithPlace(@Param("label") CurationLabel label, Pageable pageable);

    @Query("SELECT pp FROM PopularPlace pp JOIN FETCH pp.place pl JOIN FETCH pl.category WHERE pp.curationLabel IS NULL ORDER BY CASE WHEN pl.imageUrl IS NULL THEN 1 ELSE 0 END ASC, pp.visitCount DESC, pp.id ASC")
